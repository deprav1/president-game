 ...
 * - easy     — негатив сильно гасится (×0.40), позитив чуть усиливается (×1.05):
 ...
 *
 * Сигнатура обратносовместима: вызов scaleStatEffect(key, val) == «normal».
 */
export const scaleStatEffect = (key, val, difficulty = "normal", months = 1) => {
  if (!val) return 0;

  const base = key === "people" ? (val > 0 ? 0.82 : 1.04) : 0.95;

  let mult = base;
  if (difficulty === "easy") {
    mult = base * (val < 0 ? 0.40 : 1.05);
