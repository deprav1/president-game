...
export const CHAINS = {
  ...
  // Карта 2а — жёсткая ветка (игрок заблокировал Наружу)
  ds_arc_2_blockade: {
    id: "ds_arc_2_blockade",
    delay: 3,
    card: {
      advisor: 3,
      text: "DS_ARC_3_HARD Сенин докладывает: блокировка Наружу обошлась — граждане перешли на десятки других VPN. Трафик через запрещённые каналы вырос вдвое. Армия требует полного отключения зарубежного интернета.",
  ...
  ds_arc_4_hard_end: {
    ...
    card: {
      advisor: 6,
      text: "Стрельцова: цифровой занавес привёл к бегству 80 000 IT-специалистов. Экономика теряет $2 млрд в год. Но Сенин доволен — оппозиция не координируется. Граждане всё равно используют Наружу через спутник.",
  ...
  ds_arc_4_soft_end: {
    ...
    card: {
      advisor: 2,
      text: "Власова: Варония вошла в топ-20 стран по цифровой свободе. Молодёжь называет страну «оазисом». Наружу открыл здесь штаб-квартиру и платит налоги. Сенин недоволен — «слишком много иностранного влияния».",
...
export const CHAIN_TRIGGERS = [
  ...
  (text, side) =>
    side === "left" && text.includes("VPN-сервиса Наружу")
      ? "ds_arc_2_blockade"
      : null,

  // Арк: не вмешиваться — запускает мягкую ветку
  (text, side) =>
    side === "right" && text.includes("VPN-сервиса Наружу")
      ? "ds_arc_2_open"
      : null,
